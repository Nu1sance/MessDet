...
#######################################################################################################
        ...
        for i in range(8):
            img = mmcv.imread(image_path)
            img = mmcv.imconvert(img, 'bgr', 'rgb')
            # img = mmcv.imresize(img, (768, 768))
            img = mmcv.imrotate(img, i * 45, auto_bound=False)
            ...
            #     shown_imgs = []
            #     for featmap in flatten_featmaps:
            #         # print(featmap[0][:featmap[0].shape[0] // 8, :, :].size())
            #         split_size = featmap[0].shape[0] // 8
            #         shown_img = visualizer.draw_featmap(
            #             featmap[0][i * split_size:(i + 1) * split_size, :, :],
            #             drawn_img,
            #             channel_reduction=channel_reduction,
            #             topk=args.topk,
            #             arrangement=args.arrangement)
            ...
            #     progress_bar.update()
            #     if out_file:
            #         mmcv.imwrite(shown_imgs[..., ::-1], f'./output/angle{i}/' + filename)
            #
            #     if args.show:
            #         visualizer.show(shown_imgs)
            ...
            if out_file:
                ...
                mmcv.imwrite(shown_imgs[..., ::-1], './output/strict/' + f'{_filename}_{i}.png')
